...
/***/ (function(module, exports, __webpack_require__) {
	...
	//   window.AudioContext = window.AudioContext || window.webkitAudioContext;
	  ...
	  var audioContext = globalAudioContext;//new window.AudioContext();
	  ...
	  playlist.setWaveWidth(config.waveWidth);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _class = function () {
	  ...
	  _createClass(_class, [{
	    ...
	    key: 'setWaveWidth',
	    value: function setWaveWidth(width) {
	    ...
	    value: function setUpEventEmitter() {
		  ...
		  ee.on('recordstop', function (voicetrackname) {
	        _this2.recordstop(voicetrackname);
	      });

		  ee.on('setvtstartandnext', function (vtstart, next) {
	        _this2.setvtstartandnext(vtstart, next);
			// console.log("in setvtstartandnext", _this2.peaks, _this2);
			_this2.drawRequest();
	      });
	      ...
	      ee.on('removeTrack', function (fname) {
	        _this2.removeTrack(fname);
	        _this2.adjustTrackPlayout();
	        _this2.drawRequest();
			// console.log("removeTrack finished");
	      });
	      ...
	      ee.on('mastervolumechange', function (volume) {//0~100
	      ...
	      ee.on('newtrack', function (file, fname, width) {
	        _this2.load([{
	          src: file,
	          name: file.name
			//   start: 5
		  ...
		  ee.on('addmark', function (rawIntroUntil, rawNextStart) {
			//   console.log("addmark", _this2, rawIntroUntil, rawNextStart);
			//IF WP 11/22
			if (rawIntroUntil>0){ _this2.intro = rawIntroUntil};
			if (rawNextStart>0) {_this2.nextstart = rawNextStart;}
	      });

		  ee.on('markvtstart', function (vtstart) {
			_this2.vtstart = vtstart;
			_this2.drawRequest();
		  });
		  
		  //WP 11/22
		  ee.on('outroStart', function (outroStart) {
			_this2.outroStart = outroStart;
			_this2.drawRequest();
		  });
		  
		  ee.on('trackStart', function (trackStart) {
			_this2.theTrackStart = trackStart;
			_this2.drawRequest();
		  });
		  //WP 11/22

		  ee.on('eqsetting1', function (value) {
			_this2.eqsetting1.gain.value = value;
			eqset1 = value;
			saveEqSetting();
			console.log("eqsetting1", value);
		  });

		  ee.on('eqsetting2', function (value) {
			_this2.eqsetting2.gain.value = value;
			eqset2 = value;
			saveEqSetting();
			console.log("eqsetting2", value);
		  });

		  ee.on('eqsetting3', function (value) {
			_this2.eqsetting3.gain.value = value;
			eqset3 = value;
			saveEqSetting();
			console.log("eqsetting3", value);
		  });

		  ee.on('eqsetting4', function (value) {
			_this2.eqsetting4.gain.value = value;
			eqset4 = value;
			saveEqSetting();
			console.log("eqsetting4", value);
		  });

		  ee.on('eqsetting5', function (value) {
			_this2.eqsetting5.gain.value = value;
			eqset5 = value;
			saveEqSetting();
			console.log("eqsetting5", value);
		  });

		  ee.on('micvolumechange', function (vicvolume) {//0 ~ 100
			_this2.gainNode.gain.value = vicvolume / 100.0;
			console.log("micvolumechange", _this2.gainNode.gain.value);
		  });
	      ...
	      ee.on('trim', function () {
			...
			console.log("ee.on('trim', function ()");
	    ...
	    key: 'load',
	    value: function load(trackList, fname, width) {
		// console.log("load 3", fname, width);
		  this.waveWidth = width;
	      var _this3 = this;

		//   console.log("in load", this.intro, this.nextstart, this.vtstart);
		  this.intro = 10000;
		  this.nextstart = 10000;		  
		  ///WP 11/22
		  this.outroStart = 0;
		  this.theTrackStart = 0;
		...
		//   this.vtstart = 10000;
	      ...
	      return Promise.all(loadPromises).then(function (audioBuffers) {
			...
			//   console.log("info", info);
	          ...
	          track.calculatePeaks(/*_this3.samplesPerPixel*/_this3.waveWidth, _this3.sampleRate);
	// console.log("samplesperpixel calculated!!!!");
	          return track;
	        });
			_this3.tracks = [];
	        _this3.tracks = _this3.tracks.concat(tracks);
	        _this3.adjustDuration();
			_this3.samplesPerPixel = tracks[0].peaks.samplesPerPixel;
			 //console.log("_this3.draw(_this3.render());1111111111111111", tracks[0].peaks.samplesPerPixel, this, _this3);
	        _this3.draw(_this3.render());
			 //console.log("track added", _this3.tracks);
	        // _this3.ee.emit('audiosourcesrendered');
			_this3.ee.emit('loaded', _this3.duration);
			// console.log("ee.emit", _this3);
			// console.log(" ");
	      }).catch(function (e) {
	    ...
	    */
		...
		value: function removeTrack(fname) {
	    ...
	    */
		...
		// console.log("play now", startTime, endTime);
	    ...
	    key: 'recordstop',
	    value: function recordstop(voicetrackname) {
			// if (this.mediaRecorder && this.mediaRecorder.state === 'recording') {
			// 	this.mediaRecorder.stop();
			//   }
	    ...
	    key: 'setvtstartandnext',
	    value: function setvtstartandnext(vtstart, next) {
		...
		//   console.log("startAnimation", startTime);
	    }
	  }, {
	    key: 'stopAnimation',
	    ...
	    */
	    ...
	    value: function updateEditor(cursor) {
	      ...
	      if (this.isPlaying()) {
			...
			// console.log("timeupdate", playbackSeconds);
	        this.animationRequest = window.requestAnimationFrame(function () {
	    ...
	    value: function draw(newTree) {
	      ...
	      this.tree = newTree;
	// console.log("value: function draw(newTree)", this);
	    ...
	    value: function renderTrackSection() {
	      ...
	      var trackElements = this.tracks.map(function (track) {
	        ...
	        return track.render(_this16.getTrackRenderData({//create waveform with css in render
			  ...
			  width: _this16.waveWidth,
			  resolution: _this16.samplesPerPixel,
			  intro: _this16.intro,
			  nextstart: _this16.nextstart,
			//   markvtstart: _this16.markvtstart
			  vtstart: _this16.vtstart,
			  //WP 11/22
			  outroStart: _this16.outroStart,
			  theTrackStart: _this16.theTrackStart			  
	        }));
	      });
...
/***/ (function(module, exports) {
	...
	function InlineWorker(func, self) {
	  ...
	  function postMessage(data) {
	    setTimeout(function() {
		  ...
		  console.log("on message333333333", data);
	    }, 0);
	  }
	
	  this.self = self;
	  this.self.postMessage = postMessage;
	console.log("this.self = self;", this.self, this.self.pistMessage);
	...
	InlineWorker.prototype.postMessage = function postMessage(data) {
	  ...
	  setTimeout(function() {
		...
		console.log("post message11111111", data);
	  }, 0);
	};
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _class = function () {
	  ...
	  _createClass(_class, null, [{
		...
		// console.log("in createLoader", src)
	        ...
	        _this2.src.type.match(/video\/ogg/)) {
	          ...
	          fr.addEventListener('load', function (e) {
	            ...
	            decoderPromise.then(function (audioBuffer) {
	              resolve(audioBuffer);
				//   console.log("in load promise", audioBuffer);
	            }).catch(reject);
	          });
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _class = function (_Loader) {
	  ...
	  _createClass(_class, [{
	     ...
	     */
	    value: function load() {
		  console.log("load 2333333333333333333");
	      var _this2 = this;
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _class = function () {
	  ...
	  _createClass(_class, [{
	    ...
	    key: 'calculatePeaks',
	    value: function calculatePeaks(/*samplesPerPixel*/canvasWidth, sampleRate) {
	...
	// console.log("sampleRate", sampleRate, cueIn, cueIn,this.buffer);
	      this.setPeaks((0, _webaudioPeaks2.default)(this.buffer, /*samplesPerPixel*/canvasWidth, this.peakData.mono, cueIn, cueOut));
	    }
	  }, {
	    key: 'setPeaks',
	    value: function setPeaks(peaks) {
	      this.peaks = peaks;
		//   console.log("setPeaks222", peaks);
	    }
	  }, {
	    key: 'setState',
	    value: function setState(state) {
	...
	// console.log("start play audio", when, start, duration, this);
	    ...
	    value: function renderOverlay(data) {
	      ...
	      var config = {
	        attributes: {
	          style: 'position: absolute; top: 0; right: 0; bottom: 0; left: 0; width: ' + channelPixels + 'px; z-index: 5;'
	        }
	      };
	    ...
	    value: function renderControls(data) {
	      ...
	      return (0, _h2.default)('div.controls', {
	        attributes: {
	          style: 'height: ' + numChan * data.height + 'px; width: ' + data.controls.width + 'px; position: absolute; left: 0; z-index: 5;'
	        }
	      }, controls);
	    }
	  }, {
	    ...
	    value: function render(data) {
	      ...
	      var width = data.width;//this.peaks.length;
	    ...
	    //   }
	      ...
	      var waveformChildren = [(0, _h2.default)('div.cursor', {
	        attributes: {
	          style: 'position: absolute; width: 1px; margin: 0; padding: 0; top: 0; left: ' + playbackX + 'px; background: white; bottom: 0; z-index: 5;'
	        }
	      })];
	      ...
	      var channels = Object.keys(this.peaks.data).map(function (channelNum) {
			...
			// console.log("_this3", _this3);
	        if (_this3.fadeIn) {
			console.log("_this3.fadeIn", _this3.fadeIn);
	        ...
	        if (_this3.fadeOut) {
				console.log("_this3.fadeOut", _this3.fadeOut);
		...
		//   console.log("in render selection", _this3);
		  var xIntro = (0, _conversions.secondsToPixels)(data.intro, data.resolution, data.sampleRate);
		  waveformChildren.push((0, _h2.default)('div.intro', {
			attributes: {
			  style: 'position: absolute; width: 2px; bottom: 0; top: 0; left: ' + xIntro + 'px; background: red; z-index: 4;'
			}
		  }));
		  var xNext = (0, _conversions.secondsToPixels)(data.nextstart, data.resolution, data.sampleRate);
		  waveformChildren.push((0, _h2.default)('div.nextstart', {
			attributes: {
			  style: 'position: absolute; width: 2px; bottom: 0; top: 0; left: ' + xNext + 'px; background: rgb(0, 192, 0); z-index: 4;'
			}
		  }));
		  var xvtSTart = (0, _conversions.secondsToPixels)(data.vtstart, data.resolution, data.sampleRate);
		  waveformChildren.push((0, _h2.default)('div.vtstart', {
			attributes: {
			  style: 'position: absolute; width: 2px; bottom: 0; top: 0; left: ' + xvtSTart + 'px; background: blue; z-index: 4;'
			}
		  }));
		  
		  //WP 11/22
		  var xoutroStart = (0, _conversions.secondsToPixels)(data.outroStart, data.resolution, data.sampleRate);
		  waveformChildren.push((0, _h2.default)('div.outrostart', {
			attributes: {
			  style: 'position: absolute; width: 2px; bottom: 0; top: 0; left: ' + xoutroStart + 'px; background: orange; z-index: 4;'
			}
		  }));
		  
		  var xtrackStart = (0, _conversions.secondsToPixels)(data.theTrackStart, data.resolution, data.sampleRate);
		  waveformChildren.push((0, _h2.default)('div.trackStart', {			
			attributes: {			  				  	
			  style: 'position: absolute; width: '+ xtrackStart + 'px; bottom: 0; top: 0; left: ' + 0 + 'px; background: silver; z-index: 3; opacity: 75%;'			  	
			}
		  }));
...
/***/ (function(module, exports) {
	...
	*/
	function extractPeaks(channel, samplesPerPixel, bits) {
		...
		// console.log("chanLength, peaks 1111111", chanLength, peaks);
	    return peaks;
	}
	
	function makeMono(channelPeaks, bits) {
		// console.log("channelPeaks", channelPeaks);
	...
	*/
	module.exports = function(source, /*samplesPerPixel*/canvasWidth, isMono, cueIn, cueOut, bits) {
	    ...
	    if (typeof source.subarray === "undefined") {
	        for (c = 0; c < numChan; c++) {
				...
				samplesPerPixel = slice.length / canvasWidth;
			...
			samplesPerPixel = source.subarray(cueIn, cueOut).length / canvasWidth;
	...
	// console.log("before numPeaks = peaks[0].length / 2;", source, canvasWidth, isMono, cueIn, cueOut, bits);
...
/***/ (function(module, exports) {
	...
	var CanvasHook = function () {
	  function CanvasHook(peaks, offset, bits, color, scale, height) {
		...
		// console.log("function CanvasHook", this);
	  }
	  ...
	  _createClass(CanvasHook, [{
		...
		//   var stretch = canvas.width / len;
		//   console.log("this.peaks", this.peaks);
		//   console.log("peaks", this.peaks.length);
		//   console.log("this.offset, h2, maxValue, this.bits", this.offset, h2, maxValue, this.bits);
	      ...
	      for (var i = 0; i < len; i += 1) {
			...
			// console.log("cc, h2, i, minPeak, maxPeak", h2, i, minPeak, maxPeak);

			// for (var j = stretch * i; j < stretch * (i + 1); j += 1) {
			// 	var jdx = Math.floor(j);
			// 	// CanvasHook.drawFrame(cc, h2, jdx, minPeak, maxPeak);
			// 	//  console.log("cc, h2, jdx, minPeak, maxPeak", h2, jdx, minPeak, maxPeak);
			// }
	      }
	...
	var _class = function () {
	  ...
	  _createClass(_class, [{
	    ...
	    */
		...
		//   console.log("offlineplayout play", this, this.source);
	    }
	  }, {
	    key: 'stop',
	    value: function stop() {
...
/***/ (function(module, exports) {
	...
	// console.log("chanLength, peaks", chanLength, peaks);
	    return peaks;
	  }
	  ...
	  * @param {Number} cueOut - index in channel to end peak calculations from (non-inclusive).
	  */
	  function audioPeaks(source, canvasWidth) {
	    // var canvasWidth = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 10000;
		...
		var samplesPerPixel = source.subarray(start, end).length / canvasWidth;
	    peaks.push(extractPeaks(source.subarray(start, end), samplesPerPixel, bits));
	
	    var length = peaks[0].length / 2;
		// console.log("in audio peaks", this, canvasWidth, bits, peaks);
	  ...
	  onmessage = function onmessage(e) {
	    var peaks = audioPeaks(e.data.samples, e.data.canvasWidth);
	// console.log("in onmessage", e, e.data.samples, e.data.canvasWidth, peaks);
	    postMessage(peaks);
	  };
	};
